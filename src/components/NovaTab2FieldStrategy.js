...
// ─── Mock data — grounded in the locked 6-person MSL/Field Medical roster
// (config/vega.js): Dr. Rachel Simmons, Dr. Daniel Osorio, Dr. Priya
// Chandran, Dr. Emma Whitfield, Dr. Julian Voss, Dr. Amara Okafor.
// Territory labels replace the legacy NE/MW/SE/SW/NW placeholder regions
// throughout. ────────────────────────────────────────────────────────────

const MSL_OPTIONS = [
  'Dr. Rachel Simmons — US Northeast (PKU/MPS/Hemophilia)',
  'Dr. Daniel Osorio — US Central (Skeletal/MPS)',
  'Dr. Priya Chandran — US West (PKU/MPS)',
  'Dr. Emma Whitfield — UK & Ireland (Hemophilia/Skeletal)',
  'Dr. Julian Voss — Continental Europe (MPS/Skeletal)',
  'Dr. Amara Okafor — International/RoW (Cross-TA)',
];

const MSL_PERFORMANCE = [
  { msl: 'Dr. Rachel Simmons', territory: 'US Northeast — PKU/MPS/Hemophilia',  status: 'Excellent',    interactions: 44, target: 40, delta: '+4', qualityScore: 88, kitSignals: 5, kolsAtRisk: 1, note: 'High interaction quality. Strong Sephience-response and Roctavian US-exit focus.' },
  { msl: 'Dr. Daniel Osorio',  territory: 'US Central — Skeletal/MPS',          status: 'On track',     interactions: 39, target: 40, delta: '−1', qualityScore: 81, kitSignals: 6, kolsAtRisk: 2, note: 'SCFE precision-messaging delivery improving month-on-month.' },
  { msl: 'Dr. Priya Chandran', territory: 'US West — PKU/MPS',                  status: 'Volume gap',   interactions: 28, target: 35, delta: '−7', qualityScore: 84, kitSignals: 3, kolsAtRisk: 1, note: 'Quality strong but interactions below target — smaller West Coast account base.' },
  { msl: 'Dr. Emma Whitfield', territory: 'UK & Ireland — Hemophilia/Skeletal', status: 'Quality gap',  interactions: 41, target: 36, delta: '+5', qualityScore: 76, kitSignals: 5, kolsAtRisk: 3, note: 'Volume strong given WFH 2026 attention, quality below target. Ex-US credibility coaching needed.' },
  { msl: 'Dr. Julian Voss',    territory: 'Continental Europe — MPS/Skeletal',  status: 'On track',     interactions: 35, target: 36, delta: '−1', qualityScore: 83, kitSignals: 4, kolsAtRisk: 1, note: 'Aldurazyme co-marketing clarity and Mohnike re-engagement both progressing.' },
  { msl: 'Dr. Amara Okafor',   territory: 'International/RoW — Cross-TA',      status: 'On track',     interactions: 22, target: 24, delta: '−2', qualityScore: 80, kitSignals: 3, kolsAtRisk: 0, note: 'Cross-portfolio coordination role — lower volume by design, broadest remit.' },
];
...
const HCP_TOPICS = [
  { topic: 'Sephience-vs-KUVAN switching',        hcpCount: 46, stages: [7, 11, 17, 8, 3], avgStage: 2.8, trend: '+0.5', confirmed: 21 },
  { topic: 'SCFE-vs-achondroplasia precision',     hcpCount: 39, stages: [9, 10, 12, 6, 2], avgStage: 2.5, trend: '+0.6', confirmed: 18 },
  { topic: 'ROCTAVIAN US-exit & ex-US continuity', hcpCount: 33, stages: [12, 9, 7, 4, 1],  avgStage: 2.2, trend: '+0.3', confirmed: 12 },
];

const STAGE_LABELS = ['Unaware', 'Aware', 'Interested', 'Evaluating', 'Committed'];

// Scientific Alignment Shift — real rows are generated from the actual KOL
// roster (KOL_DATA) via config/messaging-alignment.js, so every MSL/territory
// shows shift across their full assigned KOL list. Q2 (current) is each
// KOL's real per-pillar alignment score; Q1 (previous quarter) is a
// deterministic offset off that score — same id-hash approach
// messaging-alignment.js uses for the current-quarter numbers.

const NATIONAL_METRICS = [
  { label: 'Total MSL interactions', value: '209', sub: 'this cycle' },
  { label: 'Avg quality score', value: '82', sub: 'target: 80' },
  { label: 'Insight capture rate', value: '68%', sub: '+5% vs prior' },
  { label: 'KOLs at risk', value: '8', sub: 'tier 1 / tier 2', alert: true },
];

const TERRITORY_LIST = [
  'US Northeast — PKU/MPS/Hemophilia',
  'US Central — Skeletal/MPS',
  'US West — PKU/MPS',
  'UK & Ireland — Hemophilia/Skeletal',
  'Continental Europe — MPS/Skeletal',
  'International/RoW — Cross-TA',
];

// Per-territory HCP topics (scoped subset of national data)
const TERRITORY_HCP_TOPICS = {
  'US Northeast — PKU/MPS/Hemophilia': [
    { topic: 'Sephience-vs-KUVAN switching',        hcpCount: 13, stages: [2, 3, 5, 2, 1], avgStage: 2.9, trend: '+0.6', confirmed: 7 },
    { topic: 'ROCTAVIAN US-exit transition',         hcpCount: 10, stages: [3, 3, 2, 2, 0], avgStage: 2.2, trend: '+0.4', confirmed: 4 },
    { topic: 'MPS referral-pathway gaps',             hcpCount: 7,  stages: [2, 2, 2, 1, 0], avgStage: 2.1, trend: '+0.2', confirmed: 2 },
  ],
  'US Central — Skeletal/MPS': [
    { topic: 'SCFE-vs-achondroplasia precision', hcpCount: 15, stages: [3, 4, 5, 2, 1], avgStage: 2.6, trend: '+0.7', confirmed: 8 },
    { topic: 'Hypochondroplasia readiness',       hcpCount: 9,  stages: [2, 2, 3, 2, 0], avgStage: 2.4, trend: '+0.5', confirmed: 4 },
    { topic: 'MPS referral-pathway gaps',           hcpCount: 6,  stages: [2, 2, 1, 1, 0], avgStage: 2.0, trend: '+0.1', confirmed: 2 },
  ],
  'US West — PKU/MPS': [
    { topic: 'Sephience-vs-KUVAN switching', hcpCount: 10, stages: [1, 2, 4, 2, 1], avgStage: 3.0, trend: '+0.5', confirmed: 5 },
    { topic: 'PALYNZIQ adolescent adoption',  hcpCount: 8,  stages: [1, 3, 2, 2, 0], avgStage: 2.4, trend: '+0.4', confirmed: 4 },
    { topic: 'MPS referral-pathway gaps',      hcpCount: 5,  stages: [2, 1, 1, 1, 0], avgStage: 1.9, trend: '0.0', confirmed: 1 },
  ],
  'UK & Ireland — Hemophilia/Skeletal': [
    { topic: 'ROCTAVIAN US-exit & ex-US continuity', hcpCount: 12, stages: [4, 3, 3, 1, 1], avgStage: 2.1, trend: '+0.3', confirmed: 4 },
    { topic: 'SCFE-vs-achondroplasia precision',      hcpCount: 8,  stages: [2, 2, 2, 2, 0], avgStage: 2.5, trend: '+0.5', confirmed: 3 },
    { topic: 'Sephience-vs-KUVAN switching',           hcpCount: 3,  stages: [1, 1, 1, 0, 0], avgStage: 2.0, trend: '0.0', confirmed: 1 },
  ],
  'Continental Europe — MPS/Skeletal': [
    { topic: 'Aldurazyme co-marketing clarity',   hcpCount: 9,  stages: [3, 3, 2, 1, 0], avgStage: 2.1, trend: '+0.3', confirmed: 3 },
    { topic: 'BMN 333 vs navepegritide',           hcpCount: 7,  stages: [2, 2, 2, 1, 0], avgStage: 2.1, trend: '+0.4', confirmed: 2 },
    { topic: 'SCFE-vs-achondroplasia precision',   hcpCount: 6,  stages: [2, 1, 2, 1, 0], avgStage: 2.2, trend: '+0.2', confirmed: 2 },
  ],
  'International/RoW — Cross-TA': [
    { topic: 'ROCTAVIAN US-exit & ex-US continuity', hcpCount: 6, stages: [2, 2, 1, 1, 0], avgStage: 2.2, trend: '+0.2', confirmed: 2 },
    { topic: 'MPS referral-pathway gaps',              hcpCount: 5, stages: [2, 1, 1, 1, 0], avgStage: 2.0, trend: '+0.1', confirmed: 1 },
    { topic: 'Hypochondroplasia readiness',             hcpCount: 3, stages: [1, 1, 1, 0, 0], avgStage: 2.0, trend: '0.0', confirmed: 1 },
  ],
};

const TERRITORY_KIT_COVERAGE = {
  'US Northeast — PKU/MPS/Hemophilia':  [true,  true,  true,  false, false, false, false, true],
  'US Central — Skeletal/MPS':          [false, false, false, false, true,  true,  true,  false],
  'US West — PKU/MPS':                  [true,  true,  false, false, false, false, false, false],
  'UK & Ireland — Hemophilia/Skeletal': [false, false, false, false, false, true,  false, true],
  'Continental Europe — MPS/Skeletal':  [false, false, true,  true,  false, true,  true,  false],
  'International/RoW — Cross-TA':       [false, false, false, true,  false, false, false, true],
};

const TERRITORY_BRIEFS = {
  'US Northeast — PKU/MPS/Hemophilia': 'US Northeast is the strongest-performing territory this cycle — 44 interactions, quality score 88. Dr. Rachel Simmons is driving KIT signal coverage across 5 topics, anchored on the Boston/Pittsburgh academic corridor. The US Roctavian patient-transition protocol (MO6) is the standout deliverable this cycle. No KOL engagement gaps.',
  'US Central — Skeletal/MPS': 'US Central is on track with 39 interactions and quality score 81. SCFE precision-messaging delivery (MO4) is improving fast — highest KIT signal count of any territory this cycle. Two tier-1 KOLs are at engagement risk (>45 days since last contact), including Dr. Ravi Savarirayan.',
  'US West — PKU/MPS': 'US West shows a volume gap — 28 interactions against a target of 35, reflecting a smaller West Coast account base. Quality score is strong at 84. One tier-1 KOL at risk. Dr. Priya Chandran has the highest per-interaction quality on the team.',
  'UK & Ireland — Hemophilia/Skeletal': 'UK & Ireland shows a quality gap — 41 interactions (above target) but quality score of 76, below the 80 threshold, reflecting the intensity of Roctavian US-exit conversations following WFH World Congress 2026. Three KOLs at engagement risk. Ex-US credibility coaching recommended before next cycle.',
  'Continental Europe — MPS/Skeletal': 'Continental Europe is on track with 35 interactions and quality score 83. Aldurazyme co-marketing clarity (MO3) and the Dr. Klaus Mohnike re-engagement (MO5) are both progressing. One tier-1/2 KOL at risk.',
  'International/RoW — Cross-TA': 'International/RoW is on track with 22 interactions and quality score 80 — lower volume by design given the broadest cross-portfolio remit. Dr. Amara Okafor coordinates Brazil, South Africa, Japan, and Australia coverage. No KOL engagement gaps.',
};

const MO_FIELD_DATA = {
  MO1: { interactions: 41, insights: 1, kols: 6,  impact: 'Sephience comparison card in development across US territories. Community switching pressure confirmed.' },
  MO2: { interactions: 33, insights: 1, kols: 6,  impact: 'Adolescent PALYNZIQ starter guide deployed. Field adoption discussion rate rising.' },
  MO3: { interactions: 37, insights: 2, kols: 9,  impact: 'Internal Aldurazyme clarity card accepted. MPS screening checklist in field distribution.' },
  MO4: { interactions: 71, insights: 2, kols: 10, impact: 'SCFE precision talking points accepted and deployed. Hypochondroplasia readiness checklist in progress.' },
  MO5: { interactions: 24, insights: 1, kols: 10, impact: 'BMN 333 differentiation briefing scoping underway. Mohnike re-engagement not yet scheduled.' },
  MO6: { interactions: 67, insights: 2, kols: 9,  impact: 'US transition protocol in field distribution. Ex-US credibility statement accepted and deployed.' },
};
...
function FieldBrief({ scope }) {
  const briefs = {
    national: 'National field performance is tracking at 82/100 quality score, 2 points above target. SCFE precision-messaging (MO4) shows the strongest field signal activity — 71 interactions and 2 insights captured this cycle. KOL engagement risk is elevated in UK & Ireland: 3 tier-1/2 KOLs have had no substantive contact in the past 60 days. The BMN 333 vs. navepegritide question (LP7) continues to generate lower interaction volumes than the other listening priorities — directed MSL activation recommended ahead of the EU decision.',
...
function TerritoryView() {
  const [territory, setTerritory] = useState('US Northeast — PKU/MPS/Hemophilia');
  const [openTopic, setOpenTopic] = useState(null);
...
function MSLView({ selectedMSL }) {
  ...
  return (
    <div className="space-y-7">
      ...
      <section>
        <SectionHeader icon={BookOpen} label="Listening Priorities & KITs" sub="ISP config · LP1–LP8" right={<ExportBtn />} />
        <div className="space-y-2">
          {LISTENING_PRIORITIES.map((lp) => {
            const insightCount = INSIGHTS.filter((i) => i.lpRefs?.includes(lp.id)).length;
            const isGap = insightCount === 0;
            return (
