...
function KOLManagement() {
  ...
  const productName = selectedProductFilter === 'all'
    ? 'All Products'
    : PRODUCT_OPTIONS.find(p => p.id === selectedProductFilter)?.name;
  ...
  const filteredKOLs = useMemo(() => {
    return KOL_DATA.filter(kol => {
      ...
      const matchesProduct = selectedProductFilter === 'all' || kol.productAlignment.includes(selectedProductFilter);
      return matchesSearch && matchesSpecialty && matchesTier && matchesRegion && matchesProduct;
    });
  }, [searchTerm, selectedSpecialty, selectedTier, selectedRegion, selectedProductFilter]);

  const specialties = useMemo(() => [...new Set(KOL_DATA.map(k => k.specialty))], []);
  const regions = useMemo(() => [...new Set(KOL_DATA.map(k => k.region))], []);

  const stats = useMemo(() => {
    const aligned = selectedProductFilter === 'all'
      ? KOL_DATA
      : KOL_DATA.filter(k => k.productAlignment.includes(selectedProductFilter));
    return {
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        {showFilters && (
          <div className="flex flex-wrap gap-4 pt-2 border-t border-auri-border">
            <select value={selectedProductFilter} onChange={(e) => setSelectedProductFilter(e.target.value)}
              ...
              {PRODUCT_OPTIONS.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
      ...
      {/* KOL Table */}
          ...
          <div className="text-xs text-auri-muted/60 text-right">
            Showing {filteredKOLs.length} of {KOL_DATA.length} KOLs{selectedProductFilter !== 'all' ? ` aligned with ${productName}` : ''}
          </div>
    </div>
    </>
  );
